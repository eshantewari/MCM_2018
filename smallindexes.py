...
def graph_either_profile(attributelist, given, statedata, datadic, min, state):
    ...
    # Set labels and title, notify the
...
def graph_sector_profile(sector, statedata, datadic, min, state=''):
    ...
    graph_either_profile(attributelist, sector, statedata, datadic, min, state)



def graph_source_profile(source, statedata, datadic, min, state=''):

    # Find the right attributes/factors
    attributelist = [factor for factor in statedata.columns if factor[0:2] == source and factor[2:4] in sectordic
                     and factor[-1] == 'B']

    graph_either_profile(attributelist, source, statedata, datadic, min, state)



def create_sector_profile(state, sector):
    statedata, datadic = gd.get_data(state)
    graph_sector_profile(sector, statedata, datadic, 0.01, state)

def create_source_profile(state, source):
    statedata, datadic = gd.get_data(state)
    graph_source_profile(source, statedata, datadic, 0.01, state)
